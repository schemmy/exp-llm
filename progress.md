...
| Wk | Dates | Task summary | Status | Artifact / notes |
|----|-------|--------------|--------|------------------|
| 15 | 12-06 → 12-12 | PyTorch DDP baseline on 2× A100: measure scaling efficiency on a small model as batch size / model size vary | ✅ | Scaling efficiency 96.56%→99.96% (batch 4→64), way above predicted 70-85% — GPT-2 124M's gradient volume is small enough that DDP's comm/backward overlap hides nearly all of it. comm_fraction 4.41% @ bs16 |
| 16 | 12-13 → 12-19 | DDP gradient bucketing internals; find where DDP overhead lives; sweep bucket_cap_mb | ✅ | bucket_cap_mb has no measurable effect at 124M or 774M (both <1.2%, noise). Cross-container scaling-efficiency comparison for 774M was unreliable (108.54%, physically impossible) — noise from separate Modal containers exceeded the effect size |
...
- **2026-09-20**: **Wk 15 complete — first P2 result, and it overturned the week's own prediction.** DDP scaling efficiency on 2× A100 (GPT-2 124M, synthetic tokens): 96.56% at batch=4, 99.96% at batch=64 — predicted 70-85% at batch=4, expecting communication to be a bigger fraction of a cheaper step. Actual mechanism: DDP overlaps the gradient all-reduce with backward compute, and at this model size that overlap hides nearly all of it (comm_fraction of raw CUDA time was 4.41% at batch=16, but the actual throughput loss was only 1.15% — the gap between those two numbers *is* the overlap). Two bugs hit along the way, both fixed in-place: a profiler attribute renamed by a torch version bump (`cuda_time_total` → `device_time_total`), and a scaling-efficiency formula that double-divided by 2 (first run showed ~48-50% efficiency from a math error, not from real data — the underlying measurements were fine, only the report formula was wrong). Results in `week15.md` and the new `benchmarks/README_training.md` (P2's benchmarks doc — kept separate from `benchmarks/README.md`, whose title/scope is explicitly Phase-1 single/multi-GPU *inference*).
- **2026-09-21**: **Wk 16 complete — a clean negative result, plus a method lesson.** Swept `bucket_cap_mb` (1/25/100/500 MB) at two model sizes (GPT-2 124M and GPT-2 Large 774M, ~6x the gradient volume): no measurable effect at either size (both <1.2% spread, and 774M's was non-monotonic — judged as noise, not over-interpreted). Separately tried to compute 774M's scaling efficiency (single-GPU vs. DDP) and got 108.54% — physically impossible, since DDP can only add communication cost, never remove it. Root cause: the single-GPU and DDP legs ran in separate Modal containers, and cross-container GPU throughput variance (~8.5% here) exceeded the actual effect being measured. Method lesson for future weeks: a cross-run throughput *ratio* is only reliable when the effect size is bigger than typical inter-container variance (Wk 15's DDP overhead was, at 4.41% comm_fraction; this week's apparent 774M overhead wasn't) — a same-run `torch.profiler` measurement (like Wk 15's comm_fraction) is the noise-immune way to measure it, and this week's script didn't build that for the 774M leg, a design gap not worth new GPU spend to fix retroactively. Also caught and fixed mid-week: the script's first draft never included a single-GPU 774M baseline at all, despite week16.md promising that comparison as "the number this week most wants" — added before the final runs. Sets up Wk 17: DDP's overlap mechanism held at both sizes tested, so FSDP's justification is memory capacity (774M already used 66.6GB/80GB per rank), not communication efficiency — a distinction to state plainly at the start of Wk 17 rather than assume.
