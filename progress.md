...
| 17 | 12-20 → 12-26 | FSDP full-shard on 7B model (Qwen2.5-7B); measure memory vs DDP baseline | ✅ | Single-GPU fp32 OOMs exactly where predicted (first `optimizer.step()`, 78.14/79.25 GiB used). FSDP FULL_SHARD fp32 technically fits (76.82GB/GPU) but thrashes on OOM-retry nearly every step; fp16 FULL_SHARD is clean (38.33GB/GPU, 1,098 tok/s, comm_fraction 22.91%) |
...
- **2026-09-22**: **Wk 17 complete — both genuinely-uncertain questions answered, prediction hit exactly on one, missed on the other.** Ran Qwen2.5-7B-Instruct (the same model P1's whole inference arc benchmarked) for real: single-GPU fp32 OOMs precisely where predicted — model load, optimizer creation, forward, and backward all complete; the failure lands on the first `optimizer.step()` call, where Adam lazily allocates its fp32 state (78.14/79.25 GiB used at failure). Not predicted: FSDP FULL_SHARD at the *same* fp32 precision, 2x A100, actually succeeded (76.82GB/GPU, 238.3 tok/s) — but the run logs show continuous OOM-retry-and-recover warnings across nearly the entire ~2.5-minute run, meaning it survived by thrashing against the memory ceiling on almost every step, not by comfortably fitting. The fp16 FULL_SHARD leg (loaded directly in fp16) was clean — zero OOM warnings, 38.33GB/GPU (49.9% of the fp32 figure, matching the theoretical halving almost exactly), 1,098 tok/s (4.61x the fp32 throughput — bigger than precision alone would explain, but not decomposable from these two data points since the fp32 number is itself contaminated by retry overhead; recorded as an open, not force-resolved, attribution question). comm_fraction 22.91% vs. Wk 15's DDP number (4.41%, but a different model size, 7B vs 124M, so not a controlled comparison — noted as such rather than claimed as "FSDP costs 5.2x more"). Sets up Wk 18: ZeRO-1/2 shard *less* than FULL_SHARD (ZeRO-3), so if FULL_SHARD itself is already thrashing at fp32, ZeRO-1/2 almost certainly won't fit at fp32 at all — Wk 18's sweep likely needs to move to fp16 across the board to compare sharding strategies on equal footing, a design decision this week's data made rather than a guess.
